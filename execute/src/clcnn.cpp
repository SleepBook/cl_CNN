...
clcnn::~clcnn()
{
	...
	if(device_load_num!=0)
	{
		...
		clReleaseKernel(convp_sig_avg_mc);
		clReleaseKernel(convp_sig_avg_pc);
...
{	
	//parse the .cnet file and setup necessary DS on host
	
	int i, j, k, m, n;
	net_config = fopen(net_file, "r");
	fscanf(net_config, "%d", &layer_num);
	fscanf(net_config, "%d", &input_width);
	fscanf(net_config, "%d", &input_height);
	fscanf(net_config, "%d", &input_channel);
	...
	for (i = 0; i < layer_num; i++)
	{
		...
		inter_res_dim[i][0] = 0;
		inter_res_dim[i][1] = 0;
		kernel_dim[i] = (int*)malloc(sizeof(int) * 2);
	...
	for (i = 0; i < layer_num; i++)
	{
		...
		{
			#ifdef VERBOSE
			printf("read in a FULL-Connected layer\n");
			#endif
			feature_num[i + 1] = 1;
			stride[i] = 0;
			method[i] = 0;
...
int clcnn::getInput(const char* infile)
{
	FILE* input_data;
	int i, j, k;
	input_data = fopen(infile, "r");
	int local_input_channel, local_input_width, local_input_height;
	fscanf(input_data, "%d", &local_input_channel);
	fscanf(input_data, "%d", &local_input_width);
	fscanf(input_data, "%d", &local_input_height);
...
void clcnn::load_device()
{
	...
	std::cout << "Please Spicify the Platform to Use: type in the number and enter" << std::endl;
	std::cin >> platnum;
	...
	printf("%u Devices Supported by This Platform:\n", num);
	for (i = 0; i < num; i++)
	{
...
int clcnn::exe_conv_device(int* layer_pos, int flag)
{
	...
	{
		#ifdef VERBOSE
		printf("(Followed by a Pooling Layer, Enter fuse Execution Mode\n");
		#endif
		...
		if(launch_mode == 1)
		{
			...
			err = clEnqueueNDRangeKernel(queue, convp_sig_avg_full, 2, 0, global_work_size, local_work_size, 0, NULL, &timer);			
		}
		...
		{
			#ifdef VERBOSE
			printf("\tEntering Single-Channel Mode\n");
			#endif
...
int clcnn::execute_cpu(const char* infile)
{
	if(cpu_load_num == 0)
	{
		load_cpu();
	...
	for(i=0;i<layer_num;i++)
	{
		...
		{
			#ifdef VERBOSE
			printf("executing layer %d:convlution\n",i);
			#endif
		...
		{
			#ifdef VERBOSE
			printf("executing layer %d: pooling\n",i);
			#endif
			int ker_w, ker_h;
			ker_w = kernel_dim[i][0];
			ker_h = kernel_dim[i][1];
		...
		{
			#ifdef VERBOSE
			printf("excuting layer %d: FULL-Connected\n",i);
			#endif
	...
	gettimeofday(&tv, NULL);
	endd = tv.tv_usec;
	execution_time = (float)(endd - star);
	#endif
...
int clcnn::execute_device(const char* infile)
{
	if(device_load_num == 0)
	{
		load_device();
	...
	for(i=0;i<layer_num;i++)
	{
		...
		{
			#ifdef VERBOSE
			printf("execution layer%d on OpenCL device:CONV ",i);
			#endif
		...
		{
			#ifdef VERBOSE
			printf("execution layer%d on OpenCL device: FC\n",i);
			#endif
	...
	execution_time = (float)(elapse*1e-03);
	return 0;
}
...
float clcnn::get_execution_time()
{
